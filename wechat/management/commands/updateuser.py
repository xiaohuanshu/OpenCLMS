...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for s in students:
            ...
            if userinfo:
                print 'exist'
                if userinfo['department'][0] != s.classid.wechatdepartmentid:
                    wechat_client.user.update(userinfo['userid'], department=[s.classid.wechatdepartmentid])
            else:
                print 'create'
                wechat_client.user.create('S%s' % s.studentid, s.name, s.classid.wechatdepartmentid, u'学生', gender=s.sex,
        ...
        for t in teachers:
            ...
            if userinfo:
                print 'exist'
                if sorted(userinfo['department']) != sorted(tdeps):
